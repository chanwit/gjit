...
public class Breakdown001Tests extends TestCase {
    ...
    static {
        try {
            RandomAccessFile f = new RandomAccessFile("test/alioth/partialsums/Breakdown001Subject.class","r");
            bytes = new byte[(int) f.length()];
            f.readFully(bytes);
            PreProcess p = PreProcess.perform(bytes);
            out = Optimiser.perform(p, bytes);
            Utils.dumpForCompare(bytes, out);
            c = Utils.loadClass("alioth.partialsums.Breakdown001Subject", out);
        } catch(Throwable e) {
        }
    }
